...
enum PinyinUtil {
	...
	public static final String POLYPHONE_MULTIPLE_MAPPING_FILE = "/polyphone_multiple.txt";
	...
	private SmartForest<String[]> polyphoneDictForMultiple = null;

	private int maxLen = 2;

	private int maxLenForMultiple = 2;

	PinyinUtil() {
		polyphoneDict = new SmartForest<String[]>();
		polyphoneDictForMultiple = new SmartForest<String[]>();
		loadPinyinMapping();
		...
		try {
			...
			while (null != (line = in.readLine())) {
				...
				if(pair.length==2&&StringUtil.isNotBlank(pair[1])){
					...
					polyphoneDictForMultiple.add(pair[0], pair[1].split(","));
				}
	...
	public void loadPolyphoneMapping() {
		...
		try {
			BufferedReader in = new BufferedReader(
					new InputStreamReader(new BufferedInputStream(getClass().getResourceAsStream(POLYPHONE_MULTIPLE_MAPPING_FILE)), StandardCharsets.UTF_8));
			...
			while (null != (line = in.readLine())) {
				...
				maxLenForMultiple = maxLen < pair[0].length() ? pair[0].length() : maxLen;

				polyphoneDictForMultiple.add(pair[0], pair[1].split(SPACE));
	...
	public List<String> convertForMultiple(String str,  PinyinFormatter.TYPE format) {

		if (StringUtil.isBlank(str)) {
			return Collections.emptyList();
		}

		SmartGetWord<String[]> word = polyphoneDictForMultiple.getWord(str);
